...
#### Bugfixes
- Fixed incorrect `s5cmd version` output ([#650](https://github.com/weaviate/s5cmd/pull/650))
## v2.2.0 - 21 Aug 2023

#### Features
- Added `pipe` command. ([#182](https://github.com/peak/s5cmd/issues/182))
- Added `presign` command. ([#634](https://github.com/weaviate/s5cmd/pull/634)) [@zemul](https://github.com/zemul)
...
- Upgraded minimum required Go version to 1.19. ([#583](https://github.com/weaviate/s5cmd/pull/583))
- `ListObjectsV2` S3 API is enabled for Google Cloud Storage. ([#617](https://github.com/weaviate/s5cmd/pull/617))
- Added installation instructions for FreeBSD. ([#573](https://github.com/weaviate/s5cmd/pull/573)) [@ehaupt](https://github.com/ehaupt)
- Added `ppc64le` support. ([#552](https://github.com/weaviate/s5cmd/pull/552)) [@mgiessing](https://github.com/mgiessing)
...
- Fixed a bug where `cp` command could not upload a non-regular file to remote destination. ([#618](https://github.com/weaviate/s5cmd/pull/618))
- Fixed a crash where a file or a remote object is removed or renamed after it is listed to be operated on. ([#620](https://github.com/weaviate/s5cmd/pull/620))

## v2.1.0 - 19 Jun 2023

#### Breaking changes
- Adjacent slashes in key are no longer removed when uploading to remote. Before `s5cmd cp file.txt s3://bucket/a//b///c/` would copy to `s3://bucket/a/b/c/file.txt` but now to `s3://bucket/a//b///c/file.txt`.([#459](https://github.com/weaviate/s5cmd/pull/459))
- `--endpoint-url` will not accept URLs without scheme such as `example.com`. Instead, it will give an error and ask for an url with a scheme; either `http://example.com` or `https://example.com` ([#496](https://github.com/weaviate/s5cmd/pull/496)).
...
- Added `--all-versions` flag to `ls`, `rm`, `du` and `select` subcommands to apply operation on(/over) all versions of the objects. ([#475](https://github.com/weaviate/s5cmd/pull/475))
- Added `--version-id` flag to `cat`, `cp`/`mv`, `rm`, `du`  and `select` subcommands to apply operation on(/over) a specific versions of the object. ([#475](https://github.com/weaviate/s5cmd/pull/475))
- Added `bucket-version` command to configure bucket versioning. Bucket name alone returns the bucket versioning status of the bucket. Bucket versioning can be configured with `set` flag. ([#475](https://github.com/weaviate/s5cmd/pull/475))
- Added `--raw` flag to `cat` and `select` subcommands. It disables the wildcard operations. ([#475](https://github.com/weaviate/s5cmd/pull/475))
- Added `bench.py` script under new `benchmark` folder to compare performances of two different builds of s5cmd. ([#471](https://github.com/weaviate/s5cmd/pull/471))

#### Improvements
- Disable AWS SDK logger if log level is not `trace`. ([#460](https://github.com/weaviate/s5cmd/pull/460))
- Allow adjacent slashes to be used as keys when uploading to remote. ([#459](https://github.com/weaviate/s5cmd/pull/459))
- Debian packages are provided on [releases page](https://github.com/weaviate/s5cmd/releases) ([#380](https://github.com/peak/s5cmd/issues/380))
...
- Changed cp error message to be more precise. "given object not found" error message now will also include absolute path of the file. ([#463](https://github.com/weaviate/s5cmd/pull/463))
...
- Working directory of the Docker image changed to `/aws` from `/`. ([#412](https://github.com/weaviate/s5cmd/pull/412))
...
- Print error if the commands file of `run` command is not accessible. ([#410](https://github.com/weaviate/s5cmd/pull/410))
...
- Print usage error if given log level(`--log`) is not valid. ([#430](https://github.com/weaviate/s5cmd/pull/430))
...
- Added `--cache-control` and `--expires` flags to `cp` and `mv` commands. It adds support for setting cache control and expires header to S3 objects. ([#318](https://github.com/weaviate/s5cmd/pull/318)) [@tombokombo](https://github.com/tombokombo)
...
- Added `MacPorts` installation option. ([#311](https://github.com/weaviate/s5cmd/pull/311)) [@manojkarthick](https://github.com/manojkarthick)
- Added `S3_ENDPOINT_URL` environment variable ([#343](https://github.com/weaviate/s5cmd/pull/343)) [@Dexus](https://github.com/Dexus)
- Prevent retries if a token related error is received ([#337](https://github.com/weaviate/s5cmd/pull/337))

#### Bugfixes

- Change the order of precedence in URL expansion in file system. Glob (*) expansion have precedence over directory expansion. ([#322](https://github.com/weaviate/s5cmd/pull/322))
- Fixed data race for concurrent writes for expand ([#330](https://github.com/weaviate/s5cmd/pull/330))
- Fixed concurrent writes to the flags list of run command ([#335](https://github.com/weaviate/s5cmd/pull/335))
- Fixed options usage on mv command ([#338](https://github.com/weaviate/s5cmd/pull/338))
...
- If retryable errors are received during command execution, users now can see what's happening under the hood. ([#261](https://github.com/weaviate/s5cmd/pull/261))
- Update documentation about the AWS_PROFILE environment variable. ([#275](https://github.com/weaviate/s5cmd/pull/275)) [@davebiffuk](https://github.com/davebiffuk)
...
- Fixed a transfer issue where the object path contains particular regex metacharacters. ([#111](https://github.com/weaviate/s5cmd/pull/111)) [@brendan-matroid](https://github.com/brendan-matroid)
- Correctly parse object paths that contain whitespaces in run-mode. ([#111](https://github.com/weaviate/s5cmd/pull/111)) [@brendan-matroid](https://github.com/brendan-matroid)


#### Improvements

- Retry when connection closed by S3 unexpectedly. ([#189](https://github.com/weaviate/s5cmd/pull/189)) [@eminugurkenar](https://github.com/eminugurkenar)
...
- Dropped `batch-rm` command. It was not listed in the help output. Now that we
  ...
  wildcards or multiple arguments to `s5cmd rm` command. ([#106](https://github.com/weaviate/s5cmd/pull/106))
- [Virtual host style bucket name
  ...
  will fall back to the `path-style`. ([#92](https://github.com/weaviate/s5cmd/pull/92))
...
- `-ds`, `-dw`, `-us` and `-uw` global flags are no longer available. Multipart
  ...
  replacement flags are `--concurrency | -c` and `--part-size | -p`. ([#110](https://github.com/weaviate/s5cmd/pull/110))
...
- Added [Google Cloud Storage](https://github.com/weaviate/s5cmd#google-cloud-storage-support) support. ([#81](https://github.com/peak/s5cmd/issues/81))
...
- Pre-compiled binaries are provided on [releases page](https://github.com/weaviate/s5cmd/releases) ([#21](https://github.com/peak/s5cmd/issues/21))
